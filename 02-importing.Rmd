...
The "exchange" data do not strictly fall under the umbrella of tidy dataframes (see: [Tidy Data](http://vita.had.co.nz/papers/tidy-data.pdf)). To make all subsequent coding more streamlined an a priori re-coding of the exchange dataframes is hence warranted.
...
We are often interested in analyzing the data by ICES areas, hence one can add the a variable (here called faoarea) to the haul data, based on the shooting coordinates, using the following code (TODO: Provide a link that describes the details of this process - i.e. the geo_inside-function):
```{r, eval = FALSE}
