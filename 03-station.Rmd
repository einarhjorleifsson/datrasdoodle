...
Like most fisheries data survey data are spatial in nature. So it is only natural to start to attempt to get a spatial overview of the survey coverage. As long as we do not need to do some heavy computational (nice introduction to that is (Geocomputation with R)[https://geocompr.robinlovelace.net]) the ggplot2 framework provides seamless extensions when dealing with spatial plotting in R. Here we just jump into the deep end, but a little gentler introduction can e.g. be found [here](http://www.hafro.is/~einarhj/education/tcrenv2017/pre_ggplot2.html).
...
One is often interested in getting an overview of the gridded data. Gridding is in effect the same thing as binning data when one generates a histogram except that it is two dimensional. For spatial data it is both the longitude and the latitude that is binned (for further background see: [On gridding spatial data](http://www.hafro.is/~einarhj/splatter/post/on-gridding-data/)).
