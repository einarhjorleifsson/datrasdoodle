...
Since the length data are standardized to 60 minutes it is easy to calculate the abundance per haul.
...
Now the latter value only refers to the number of hauls where some fish was recorded, while the total number of stations in the full dataset is the number of rows in the `hh`-dataframe (`r nrow(hh)` hauls). The difference represents the number of zero stations. Now in order to include the stations with zero catch we simply do:
...
reveals that we have 27577 valid hauls with fish and 131 valid hauls where no fish was caught.
...
The ggplot2 package has a very nice (but little known) summary statistics that comes to the rescue:
...
A minor tweaking in the code would also allow us to adress the question on the mean number of species that have been caught per haul (or is it only the mean of the species where length measurments are taken??) through time:
```{r}
hh %>% 
  filter(haulval == "V") %>% 
  left_join(hl %>% 
