...
by.haul %>% 
  ...
  labs(x = NULL, y = NULL,
       colour = "Quarter",
       title = paste0(Latin, ": Mean number of per 1 hour haul"))
```
...
Here we have most hauls with relatively small or zero catches with occasional hauls with very large catches resulting in a highly negatively scewed distribution. The ggplot2 package has a very nice summary statistics (bootstrap mean and standard error) that comes to the rescue:
...
So we are now able to calculate and plot the bootstrap means:
```{r}
by.haul %>% 
  mutate(year = year + 0.25 * quarter - 0.125) %>% 
  ggplot(aes(year, wt, colour = factor(quarter))) +
...
Similarly as one rasterized temperature in the haul chapter we could rasterize the biomass and plot it according to:
```{r, fig.height = 8}
by.square <-
  by.haul %>% 
...
Must say that for the historical part the zero catch squares (for plaice) are a bit intriguing - is it a bug in the code, the way things were recorded in different cruises or a reflection of biological truth?
...
So we started with a haul dataframe with `r hh %>% filter(haulval == "V") %>% nrow()` observations and ending up with the "all" dataframe of some `r nrow(all)` records. This is a result of the unique length classes for the selected species is `r length(unique(by.haul.positive$length))`.
...
Take note that we can easily repeat the calculation of catch-per-unit-effort by year as done in the first subsection above using the "by.haul" dataframe above (not run):
```{r, eval = FALSE}
by.haul %>% 
