...
Since in the tidying process all length data were standardized to 60 minutes haul time it is easy to calculate the standardized abundance per haul.
...
A demonstration of the join-function family is provided using some simple data in [On joins](#joins}
...
Here we have most hauls with relatively small catch with occational hauls with very large catches resulting in a highly negatively scewed distribution. The ggplot2 package has a very nice summary statistics (bootstrap mean and standard error) that comes to the rescue:
...
One can now obtain a quick overview of the catch distribution:
...
by.haul %>% 
  ...
  facet_wrap(~ quarter) +
  labs(x = NULL, y = NULL, size = "Abundance",
       title = "NS-IBTS 2017",
       subtitle = "Catch distribution - No. fish per 60 minute haul")
```

If interested in rough biomass distribution the code can easily be ammended, using isometric length-weight relationship:
...
hh %>% 
  ...
  ggplot() +
  geom_polygon(data = m, aes(long, lat, group = group), fill = "grey") +
  geom_point(aes(shootlong, shootlat, size = b),
             colour = "red", alpha = 0.3) + 
  scale_size_area(max_size = 10) +
  coord_quickmap(xlim = range(by.haul$shootlong), ylim = range(by.haul$shootlat)) +
  facet_wrap(~ quarter) +
  labs(x = NULL, y = NULL, size = "Biomass",
       title = "NS-IBTS 2017",
       subtitle = "Catch distribution - Kg per 60 minute haul")
```
