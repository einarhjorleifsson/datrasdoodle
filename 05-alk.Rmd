...
```
This message was given because varible n.age has on record with NA. It so happens that it is the first record so we can see it by:
```{r}
glimpse(d)
```
...
mldata <- 
  alk %>% 
  rename(lngtclass = length) %>% 
  select(-p) %>% 
  mlogit::mlogit.data(varying=NULL,choice='age',shape='wide')
m <- mlogit::mlogit(age~1 | lngtclass, data = mldata, reflevel="1")
...
m <-
  ...
  select(-p) %>% 
  mlogit::mlogit.data(varying=NULL,choice='age',shape='wide') %>% 
  mlogit::mlogit(age~1 | lngtclass, data = ., reflevel="1")


tidy_mlogit <- function(m, lengths = 1:130, ages = 0:20) {
...
alk.model <- 
  ...
  tidy_mlogit(lengths = c(min(lfs$length):max(lfs$length)), 
              ages =    c(min(alk$age):max(alk$age)))
```
...
Let's make a comparision between this model and the empirical observations:
...
... hmmm, something rotten in the State of Denmark - code or model needs revisiting ...

... once solved, next step. assign the alk.model to the length distribution ...
