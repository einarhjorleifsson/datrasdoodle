...
One of the most common task in fisheries science it to convert the length frequency estimates of the catch (abundance) to weights. In the section on [Length data](#length) we used a rough guess for the parameter of the length-weight relationship $\alpha = 0.00001$ and $\beta = 3$). The mathematical relationship of weight (W) as a function of length (L) is:

$$W = \alpha L^{\beta}$$
Normally the parameters are estimated based on the log-transformed data^[NOTE: This has more to do with normalizing the variance in the dependent variable along the independent variable than linear models being more easy to fit (in the old days).], i.e.:
