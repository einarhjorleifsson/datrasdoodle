...
We see in the figure above that something is rotten in the state of Denmark, "old" fish but with very small length. This is likely an error in the lngtcode field
...
d <- 
  ca %>% 
  right_join(hh %>% filter(haulval == "V", !is.na(nsarea)) %>% select(id)) %>% 
  id_separate() %>% 
