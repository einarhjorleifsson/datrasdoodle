...
In fact the *a priori* sampling strategy for age determination is frequently base on some substrata (such as the nine North Sea roundfish areas) rather than on a single haul. Within each strata the objective is to sample some minimum number within each predetermined length bins (1 cm, 5 cm , ...). This strategy differs from the length sampling which is either a full census from the catch or if sub-sampling is warranted supposed to be a random sample from the catch.
...
The observations (rows) are the number of fish observed for a species, sex and maturity and age within a station. Ergo, within a haul for a particular species we could have more than one observations within a length group, this being because we may have different sex and maturities within the length group.

Those details aside, lets get an appreciation of the difference in the length distribution from the length dataframe and the age data frame:
...
bind_rows(lfs, alk) %>% 
  ...
  labs(x = "Length",
       y = "Number of fish",
       title = "Length distributions from the age and the length sampling",
       subtitle = paste(Year, Quarter, NSarea, Latin))
```

Besides the obvious difference in the total amount of fish measured, we observed that the general shape of the length frequency distributions is totally different from the "two datasources". Here the data collected for age have more or less a uniform distribution while a clear modality is observed in the length frequency data. This is because in the former case, length bins that are less frequent in the catch are, by design, sampled with a higher frequency for age determination than the most dominant length classes. One kind of wants to fill up each length class with some minimum number of fish, but no more than that.

Again, this is because the objective of the age sampling is not to generate a representative distribution of age in the survey samples. But to generate an age-length-key which is then used to split the length frequency distribution into age distribution.
...
What we have is that for each length class we have observations (points) for the probility within each length class for a particular age class. The sum within a length group is by definition always **one**. Lets take the 70 cm length class as an example:
...
Lets visualize jointly the  length frequency and age-length data for one species in one survey:
...
Rather than first grouping all the length measurements from a survey and then apply the alk-key as done above, one could use the latter to split the length distribution in each haul into age:
```{r}
by.haul.age <-
  by.haul %>% 
...
A visual representation of abundance distribution by age:
```{r}
by.haul.age %>% 
  filter(age %in% 0:3) %>% 
...
```{r}
#We can now also look at the overall age distribution:
...
We can now use the modeled alk's to the length frequency distribution:
