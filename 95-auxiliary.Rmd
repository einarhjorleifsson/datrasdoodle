...
Lets look at the 2 most common species in the length database by each speccodetype:
```{r}
hl <- 
...
hl %>% 
  group_by(speccodetype, speccode) %>% 
  summarise(n = n()) %>%
  arrange(desc(n)) %>% 
...
**Does the valid_aphia not match speccode when speccodetype is "W"?**

```{r}
hl %>% 
  filter(speccodetype == "W") %>% 
  mutate(same = ifelse(speccode == valid_aphia, TRUE, FALSE)) %>% 
  group_by(same) %>% 
  count()
```
