...
An ICES r-course on the DATRAS data was held in the 20XX. The material can be found on [Google Code Archive](https://code.google.com/archive/p/datras). The course relied heavilily on an R-packages called DATRAS, located on the archieve site.

Download practicals:

```{r eval = FALSE}
urlbase <- "https://storage.googleapis.com/google-code-archive-downloads/v2/code.google.com/datras/"
n <- 1:5
for(i in 1:5) {
  download.file(url = paste0(urlbase, "Practical0", n[i], ".r"),
                destfile = paste0("R/ices-course/Practical0", n[i], ".R"),
                                  mode = "w")
}
```
