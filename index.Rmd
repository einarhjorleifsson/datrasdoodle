...
The process that leads to fisheries advice involves multiple steps both on data munging and use (and potentially abuse) of models. The process from raw data to advice can in many cases be described as a low-pass filter being passed to another low-pass filter passed on to yet another low-pass filter. During these steps the information in as well as the limitation of the raw data is sometimes lost. Making inference/explaining how an advice relates to the raw data quite often difficult, if not impossible.
...
Mind you, there is an R package out there ([DATRAS](https://github.com/DTUAqua/DATRAS/tree/master/DATRAS)) that has more or less every functionality needed to get the job done when analyzing DATRAS data. It is for all I have checked robust and succinct. It has a however only a rudimentary introductory [wiki](https://www.rforge.net/DATRAS/Tutorial.html).

The motivation for the scribbles here was primarily to explore and analyse survey data using the tidyverse verbs and workflow^[The scribbles are not to be considered as teaching the basics of the tidyverse coding framework, for that read [R for data science](http://r4ds.had.co.nz/) by @wickham2016r.]., with specific esoteric functions related to a particular kind of data format kept to the absolute bare minimum. Kind of a testing ground to see how far one could get without leaving the environment.
