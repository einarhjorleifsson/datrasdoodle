...
The process that leads to advice involves multiple steps both on data munging and use of models. Ultimately this process is often a low-pass filter being passed to another low-pass filter passed on to yet another low-pass filter on the raw data. During these steps the information in the raw data is sometimes lost. Making inference/explaining how an advice relates to the raw data quite often difficult, if not impossible.
...
The scribbles are not to be considered as teaching the basics of the tidyverse coding framework, for that read [R for data science](http://r4ds.had.co.nz/) by (@wickham2016r)
